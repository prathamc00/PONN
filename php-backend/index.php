...
$routes = [
    ...
    ['GET',  '#^/api/courses/(?P<id>\d+)$#',       function($m) { optionalAuth(); getCourseById($m['id']); }],
    ['PUT',  '#^/api/courses/(?P<id>\d+)$#',       function($m) { protect(); staffOnly(); updateCourse($m['id']); }],
    ['DELETE','#^/api/courses/(?P<id>\d+)$#',      function($m) { protect(); staffOnly(); deleteCourse($m['id']); }],
    ['GET',  '#^/api/courses/(?P<id>\d+)/modules$#',      function($m) { protect(); getModules($m['id']); }],
    ['POST', '#^/api/courses/(?P<id>\d+)/modules$#',      function($m) { protect(); staffOnly(); addModule($m['id']); }],
    ['PUT',  '#^/api/courses/(?P<id>\d+)/modules/(?P<moduleId>[^/]+)$#', function($m) { protect(); staffOnly(); updateModule($m['id'], $m['moduleId']); }],
    ['DELETE','#^/api/courses/(?P<id>\d+)/modules/(?P<moduleId>[^/]+)$#',function($m) { protect(); staffOnly(); deleteModule($m['id'], $m['moduleId']); }],
    ['PUT',  '#^/api/courses/(?P<id>\d+)/modules/reorder$#', function($m) { protect(); staffOnly(); reorderModules($m['id']); }],
    ['POST', '#^/api/courses/(?P<id>\d+)/enroll$#',   function($m) { protect(); enrollCourse($m['id']); }],
    ['DELETE','#^/api/courses/(?P<id>\d+)/enroll$#',  function($m) { protect(); unenrollCourse($m['id']); }],
    ['GET',  '#^/api/courses/(?P<id>\d+)/progress$#', function($m) { protect(); getCourseProgress($m['id']); }],
    ['POST', '#^/api/courses/(?P<id>\d+)/modules/(?P<moduleId>[^/]+)/complete$#', function($m) { protect(); markModuleComplete($m['id'], $m['moduleId']); }],
    ...
    ['GET',  '#^/api/assignments/(?P<id>\d+)$#',       function($m) { protect(); getAssignmentById($m['id']); }],
    ['PUT',  '#^/api/assignments/(?P<id>\d+)$#',       function($m) { protect(); staffOnly(); updateAssignment($m['id']); }],
    ['DELETE','#^/api/assignments/(?P<id>\d+)$#',      function($m) { protect(); staffOnly(); deleteAssignment($m['id']); }],
    ['POST', '#^/api/assignments/(?P<id>\d+)/submit$#',function($m) { protect(); submitAssignment($m['id']); }],
    ['GET',  '#^/api/assignments/(?P<id>\d+)/submissions$#', function($m) { protect(); staffOnly(); getSubmissions($m['id']); }],

    // ----- SUBMISSIONS --------------------------------------------------------
    ['PUT',  '#^/api/submissions/(?P<id>\d+)/grade$#', function($m) { protect(); staffOnly(); gradeSubmission($m['id']); }],
    ...
    ['GET',  '#^/api/tests/(?P<id>\d+)$#',         function($m) { protect(); getTestById($m['id']); }],
    ['PUT',  '#^/api/tests/(?P<id>\d+)$#',         function($m) { protect(); staffOnly(); updateTest($m['id']); }],
    ['DELETE','#^/api/tests/(?P<id>\d+)$#',        function($m) { protect(); staffOnly(); deleteTest($m['id']); }],
    ['POST', '#^/api/tests/(?P<id>\d+)/start$#',   function($m) { protect(); startQuiz($m['id']); }],
    ['POST', '#^/api/tests/(?P<id>\d+)/submit$#',  function($m) { protect(); submitQuiz($m['id']); }],
    ['POST', '#^/api/tests/(?P<id>\d+)/retake$#',  function($m) { protect(); retakeQuiz($m['id']); }],
    ['GET',  '#^/api/tests/(?P<id>\d+)/results$#', function($m) { protect(); staffOnly(); getQuizResults($m['id']); }],
    ...
    ['GET',  '#^/api/certificates/(?P<id>\d+)$#',       function($m) { protect(); getCertificateById($m['id']); }],
    ['DELETE','#^/api/certificates/(?P<id>\d+)$#',      function($m) { protect(); adminOnly(); deleteCertificate($m['id']); }],

    // ----- USERS --------------------------------------------------------------
    ['GET',    '#^/api/users$#',                               function() { protect(); adminOnly(); getUsers(); }],
    ['POST',   '#^/api/users/instructor$#',                    function() { protect(); adminOnly(); addInstructor(); }],
    ['DELETE', '#^/api/users/(?P<id>\d+)$#',                   function($m) { protect(); adminOnly(); deleteUser($m['id']); }],
    ['PUT',    '#^/api/users/(?P<id>\d+)/approval$#',          function($m) { protect(); adminOnly(); updateInstructorStatus($m['id']); }],
    ['PUT',    '#^/api/users/(?P<id>\d+)/aadhaar-verify$#',    function($m) { protect(); adminOnly(); verifyAadhaar($m['id']); }],
    ...
    ['GET',  '#^/api/attendance/course/(?P<courseId>\d+)$#',      function($m) { protect(); staffOnly(); getCourseAttendance($m['courseId']); }],
